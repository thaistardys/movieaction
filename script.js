const firebaseConfig = { 
  apiKey: "AIzaSyATJLdOEAUrAVRDbzk0HLŌBrnUyHv6hwY",
...
document.addEventListener('DOMContentLoaded', () => { 
  ...
  loginForm.addEventListener('submit', (e) => { 
    e.preventDefault(); 
    if (!auth) { 
      loginFeedback.textContent = "Serviço indisponível no momento."; 
  ...
  function listenToFirestoreMovies() { 
    if (db) {
      db.collection('movies').onSnapshot(s => { 
        currentMovies = []; 
        s.forEach(doc => currentMovies.push({ id: doc.id, ...doc.data() })); 
        renderApp(); 
      }); 
    }
  } 
        ...
        card.innerHTML = ` 
          ...
          <div class="card-body"> 
            ...
            <div class="button-group"> 
              <button type="button" class="btn-action btn-watched ${isWatched ? 'active-watched' : ''}" data-id="${movie.id}">Assistido ✔</button>
  ...
  weeksContainer.addEventListener('click', (e) => { 
    ...
    if (btnAction) { 
      db.collection('movies').doc(btnAction.dataset.id).update({ 
        watched: btnAction.classList.contains('btn-watched') 
      }); 
      return; 
    } 
    ...
    if (btnEdit) { 
      const movie = currentMovies.find(m => m.id === btnEdit.dataset.id); 
      if (!movie) return; 
    ...
    if (btnDelete) { 
      movieToDeleteId = btnDelete.dataset.id; 
      deleteModal.classList.add('is-active'); 
    }
  }); 

  btnConfirmDelete.addEventListener('click', () => { 
    if (db && movieToDeleteId) {
      db.collection('movies').doc(movieToDeleteId).delete().then(() => { 
  ...
  movieForm.addEventListener('submit', (e) => { 
    ...
    if (id) { 
      db.collection('movies').doc(id).update(data).then(() => movieFormModal.classList.remove('is-active')); 
    } else { 
      data.watched = false; 
      data.createdAt = firebase.firestore.FieldValue.serverTimestamp(); 
      db.collection('movies').add(data).then(() => movieFormModal.classList.remove('is-active')); 
    }
  }); 
  ...
  function updateProgress(w, t) { 
    const p = t > 0 ? Math.round((w / t) * 100) : 0; 
    progressBar.style.width = `${p}%`; 
    progressText.textContent = `${w} / ${t} assistidos (${p}%)`; 
    if (t > 0 && w === t) { 
      setTimeout(() => victoryModal.classList.add('is-active'), 1000); 
    } 
  } 

  searchInput.addEventListener('input', (e) => { 
    searchQuery = e.target.value; 
    btnClearSearch.classList.toggle('is-active', !!searchQuery); 
    renderApp(); 
  });
  ...
  const initAuthActions = () => {
    ...
    if (forgotForm) { 
      forgotForm.addEventListener('submit', (e) => { 
        ...
        auth.sendPasswordResetEmail(forgotEmail.value.trim()) 
          .then(() => { 
            forgotFeedback.textContent = "E-mail de reset enviado. Verifique sua caixa de entrada ou spam."; 
            forgotFeedback.className = "is-success"; 
            setTimeout(() => { if (forgotModal) forgotModal.classList.remove('is-active'); }, 5000); 
          }) 
          .catch(() => { 
            forgotFeedback.textContent = "Erro ao enviar e-mail. Verifique o endereço."; 
            forgotFeedback.className = "is-error"; 
          }); 
      }); 
    } 
    ...
    if (registerForm) { 
      registerForm.addEventListener('submit', (e) => { 
        e.preventDefault(); 
        if (!auth) return; 
        if (registerPass.value.trim().length < 6) { 
          registerFeedback.textContent = "A senha deve conter pelo menos 6 caracteres."; 
          registerFeedback.className = "is-error"; 
          return; 
        } 
        auth.createUserWithEmailAndPassword(registerEmail.value.trim(), registerPass.value.trim()) 
          .then(() => { 
            registerFeedback.textContent = "Conta criada com sucesso! Entrando..."; 
            registerFeedback.className = "is-success"; 
            setTimeout(() => { if (registerModal) registerModal.classList.remove('is-active'); }, 1500); 
          })
          .catch(err => { 
            registerFeedback.textContent = err.code === 'auth/email-already-in-use' ? "Este e-mail já está em uso." : "Erro ao criar conta."; 
            registerFeedback.className = "is-error"; 
          }); 
      }); 
    } 
